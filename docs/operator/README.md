...
`AIExperiment` is the user-facing resource. Users describe the model,
dataset, batch size, epochs, priority, and precision; the controller decides
how to run it.

Expected lifecycle:

1. Read the `AIExperiment` spec.
...
Generated resources should be owned by the `AIExperiment` so Kubernetes garbage
